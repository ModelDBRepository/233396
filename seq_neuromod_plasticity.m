...
rew1_flag=1;  %reward 1 in the top-right corner                                                                 %reward in the circle centered in the origin
...
subplot(2,2,1)
reward_plot = plot(c(1)+r_goal*cos(-pi:2*pi/100:pi), c(2)+r_goal*sin(-pi:2*pi/100:pi)); %plot reward 1 
hold on
plot(c2(1)+r_goal2*cos(-pi:2*pi/100:pi), c2(2)+r_goal2*sin(-pi:2*pi/100:pi)) %plot reward 2
point_plot = plot(starting_position(1),starting_position(2), '.r', 'MarkerSize',10); %plot initial starting point

%plot walls
line([bounds_x(1) bounds_x(1)], [bounds_y(1) bounds_y(2)]); 
line([bounds_x(2) bounds_x(2)], [bounds_y(1) bounds_y(2)]);
line([bounds_x(1) bounds_x(2)], [bounds_y(1) bounds_y(1)]);
line([bounds_x(1) bounds_x(2)], [bounds_y(2) bounds_y(2)]);
axis([-2 2 -2 2])
...
while i<T_max*Trials
    ...
    if t==0
        ...
        if plot_flag==1
            ...
            subplot(2,2,4)
            f4=quiver(pc(:,1), pc(:,2), ac(1,:)', ac(2,:)', 'linewidth', 2);
            axis([-2 2 -2 2])
